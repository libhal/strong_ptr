...
int main()
{
  ...
  "const_conversion"_test = [&] {
    ...
    expect(nothrow([&] {
      int result = const_convert_test(opt);  // Should work with const optional
      expect(that % 42 == result);
    }));
  };
  ...
  "optional_ptr polymorphism"_test = [&] {
    auto derived = make_strong_ptr<derived_class>(test_allocator, 42);
