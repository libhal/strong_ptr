...
void enable_strong_from_this_test()
{
  ...
  "basic_functionality"_test = [&] {
    ...
    expect(that % 2 == obj.use_count()) << "Should have two strong references";
  ...
  "const_functionality"_test = [&] {
    ...
    expect(not weak_const.expired()) << "Const weak reference should be valid";
  };
  ...
  "copy_semantics"_test = [&] {
    ...
    expect(not weak1.expired()) << "First weak reference should still be valid";
