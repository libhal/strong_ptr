...
def main():
    ...
    if not os.path.isfile(compile_commands):
        print(f"ERROR: Could not find compile_commands.json in {script_dir}", file=sys.stderr)
        sys.exit(1)
    ...
    if not data:
        print(f"ERROR: Could not extract clangd path from {compile_commands}", file=sys.stderr)
        sys.exit(1)
    ...
    if not (clangd_path.exists() and os.access(clangd_path, os.X_OK)):
        print(f"ERROR: Could not find clangd at {clangd_path}", file=sys.stderr)
        sys.exit(1)
